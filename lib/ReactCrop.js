...
class ReactCrop extends React.Component {
	...
	getStyles() {
		...
		let marchingAntsColour = style.marchingAntsColour || 'rgba(255,255,255,0.7)';
		let marchingAntsAltColour = style.marchingAntsAltColour || 'rgba(0,0,0,0.7)';

		let dragHandleWidth = style.dragHandleWidth || 9;
		let dragHandleHeight = style.dragHandleHeight || 9;
		let dragHandleBackgroundColour = style.dragHandleBackgroundColour || 'rgba(0,0,0,0.2)';
		let dragHandleBorder = style.dragHandleBorder || '1px solid rgba(255,255,255,0.7)';
	...
	componentDidMount() {
		this.onDocMouseTouchMoveHandler = event => this.onDocMouseTouchMove(event);
		this.onDocMouseTouchEndHandler = event => this.onDocMouseTouchEnd(event);
	...
	onComponentMouseTouchDown(e) {
		...
		let imageOffset = this.getElementOffset(this.image);
		let xPc = (clientPos.x - imageOffset.left) / this.image.width * 100;
		let yPc = (clientPos.y - imageOffset.top) / this.image.height * 100;
	...
	createCropSelection(newCrop, fixedAspect) {
		...
		return (
				<div ref={ref => this.cropSelect = ref}
					 style={objectAssign(styles.cropSelection, this.getCropStyle())}
					 onMouseDown={event => this.onCropMouseTouchDown(event)}
					 onTouchStart={event => this.onCropMouseTouchDown(event)}>

					{!newCrop &&
					<div ref={ref => this.cropHandles = ref}>
						{!fixedAspect &&
						<div ref={ref => this.fixedAspectHandles = ref}>
	...
	render() {
		...
		let fixedAspect = !!(this.state.crop && this.state.crop.aspect);
		...
		return (
				<div
						...
						onTouchStart={event => this.onComponentMouseTouchDown(event)}
						onMouseDown={event => this.onComponentMouseTouchDown(event)}
						tabIndex="1"
						onKeyDown={event => this.onComponentKeyDown(event)}>

					<style scoped={true} type="text/css">
						{styles.marchingAntsAnimation}
					</style>

					<img ref={ref => this.image = ref} style={styles.image} src={this.props.src} onLoad={event => this.onImageLoad(event)} />

					<div style={styles.cropWrapper}>
						<img ref={ref => this.imageCopy = ref} style={objectAssign(styles.imageCopy, imageClipStyle)} src={this.props.src} />
						{cropSelection}
					</div>
